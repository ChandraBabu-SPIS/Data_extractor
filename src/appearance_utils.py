    ...
    for page in pdf.pages[:-2]:
        ...
        if line_identifier1 in text:
            ...
            for index, i in enumerate(table):
                if i[0] == "DESCRIPTION":
                    #print("des:", i)
                    # Remove all elements after the current index
                    table = table[:index]
            print("new_table:",table)
            df1= pd.DataFrame(table[1:], columns=["SPECIFICATION","METHOD"])
            specification_df = pd.concat([specification_df, df1], axis=0)
...
if __name__=="__main__":
    file = "C:\\Users\\admin\\Downloads\\SPEC-0363 Ruxolitinib Tablets.pdf"
