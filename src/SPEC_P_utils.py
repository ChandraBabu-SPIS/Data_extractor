...
def extract_spec_images(pdf, output_dir='', min_image_size_kb=20,  min_width=None, min_height=None):
    if not os.path.exists(output_dir):
        os.makedirs(output_dir)
    ...
    for page_number, page in enumerate(pdf.pages):
        ...
        for img in images_on_page:
            image_path = os.path.join(output_dir, f'page_{page_number + 1}image{image_index + 1}.png')
            img.save(image_path)
            # Ensure the title index is within the bounds of the titles list
            title = titles[title_l]
          
            data.append({'image_title': title, 'image_path': image_path, 'image': img})
            
            # Increment title index for the next image
            title_l += 1
            
        #print(data)
        print(image_path,"iiiiiiiiiiii")
...
if __name__=="__main__":
    ...
    appearance_df = extract_spec_images(pdf)
